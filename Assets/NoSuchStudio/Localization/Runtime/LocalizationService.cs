...
namespace NoSuchStudio.Localization {
    ...
    public partial class LocalizationService : Service<LocalizationService> {
        ...
        /// <summary>
        /// Tries to select the provided locale. If the locale is not in database, will fall back to neutral and default locales if allowed by parameters.
        /// </summary>
        /// <param name="localeName">name of locale to switch to</param>
        /// <param name="useNeutralIfFailed">try the neutral locale if exact locale fails</param>
        /// <param name="useDefaultIfFailed">try the default locale if previous resorts fail</param>
        /// <returns>true if locale was changed as a result of this call.</returns>
        private bool SelectLocale(string localeName, bool useNeutralIfFailed = true, bool useDefaultIfFailed = true) {
            if (!_localeDatabase.ContainsLocale(localeName)) {
                if (useNeutralIfFailed) {
                    LogLog($"locale {localeName} not in db, trying neutral locale...");
                    return SelectLocale(Locale.LanguageInLocaleName(localeName), false, useDefaultIfFailed);
                } else if (useDefaultIfFailed) {
                    LogLog($"locale {localeName} not in db, trying default locale...");
                    return SelectLocale(_defaultLocale, true, false);
                } else {
                    LogLog($"locale {localeName} not in db, giving up.");
                    return false;
                }
            }
            ...
            if (_locales.Contains(locale)) {
                ...
                LogLog($"locale {localeName} is not enabled... trying its neutral locale {locale.Parent}...");
                return SelectLocale(locale.Parent, true, useDefaultIfFailed);
            } else {
                if (useDefaultIfFailed) {
                    LogLog($"locale {localeName} is not enabled in LocalizationService, trying default locale...");
                    return SelectLocale(_defaultLocale, true, false);
                } else {
                    LogLog($"locale {localeName} is not enabled in LocalizationService, giving up.");
                    return false;
                }
            }
        }
        ...
        private void DoRemoveLocalizationSource(ITranslationSource source) {
            List<ValueTuple<string, string>> removeList = new List<ValueTuple<string, string>>();
            foreach (var plts in _translationSources) {
                foreach (var lts in plts.Value) {
                    if (lts.Value == source) removeList.Add((plts.Key, lts.Key));
                }
            }
            removeList.ForEach((ValueTuple<string, string> pl) => DoRemoveLocalizationSource(pl.Item1, pl.Item2, source));
        }
