...
static public class GameObjectTypeLogging {
    static public string StageInformationStr(GameObject go) {
        // First check if input GameObject is persistent before checking what stage the GameObject is in 
        if (EditorUtility.IsPersistent(go)) {
            if (!PrefabUtility.IsPartOfPrefabAsset(go)) {
                return "The GameObject is a temporary object created during import. OnValidate() is called two times with a temporary object during import: First time is when saving cloned objects to .prefab file. Second event is when reading .prefab file objects during import";
            } else {
                return "GameObject is part of an imported Prefab Asset (from the Library folder)";
            }
        }
        ...
        if (currentStage == mainStage) {
            if (PrefabUtility.IsPartOfPrefabInstance(go)) {
                ...
                return string.Format("GameObject is part of a Prefab Instance in the MainStage and is of type: {0}. It comes from the prefab asset: {1}", type, path);
            } else {
                return "GameObject is a plain GameObject in the MainStage";
            }
        } else {
            ...
            if (prefabStage != null) {
                if (PrefabUtility.IsPartOfPrefabInstance(go)) {
                    ...
                    return string.Format("GameObject is in a PrefabStage. The GameObject is part of a nested Prefab Instance and is of type: {0}. The opened Prefab asset is: {1} and the nested Prefab asset is: {2}", type, prefabStage.prefabAssetPath, nestedPrefabPath);
                } else {
                    ...
                    return string.Format("GameObject is in a PrefabStage. The opened Prefab is of type: {0}. The GameObject comes from the prefab asset: {1}", type, prefabStage.prefabAssetPath);
                }
            } else if (EditorSceneManager.IsPreviewSceneObject(go)) {
                return "GameObject is not in the MainStage, nor in a PrefabStage. But it is in a PreviewScene so could be used for Preview rendering or other utilities.";
            } else {
                return "Unknown GameObject Info";
            }
        }
    }

    static public string PrefabInformationStr(GameObject go) {
